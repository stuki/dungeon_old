...
dotnet restore "$DEPLOYMENT_SOURCE/dungeon-api/dungeon-api.csproj"
exitWithMessageOnError "dotnet restore failed"

# 2. Build and publish
dotnet publish "$DEPLOYMENT_SOURCE/dungeon-api/dungeon-api.csproj" --output "$DEPLOYMENT_TEMP" --configuration Release
